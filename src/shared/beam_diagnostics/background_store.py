...
def subtract_background(image, background) -> np.ndarray:
    image_array = np.asarray(image, dtype=float)
    background_array = validate_background_image(background, image_array.shape)
    if image_array.ndim != 2 or not np.all(np.isfinite(image_array)):
        raise BackgroundStoreError("Beam image must be a finite two-dimensional array.")
    return np.maximum(image_array - background_array, 0.0)
