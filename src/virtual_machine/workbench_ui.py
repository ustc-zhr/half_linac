...
class WorkbenchMixin:
    ...
    def _build_workbench(self):
        ...
        beamline_card, beamline_layout = self._workbench_card()
        beamline_layout.addWidget(self.beamline)
        self.body_splitter.addWidget(beamline_card)
        ...
        screen_title.setProperty('role', 'cardTitle')
        screen_header.addWidget(screen_title)
        screen_header.addWidget(self.screen_choice, 1)
        sl.addLayout(screen_header)
        ...
        result_row.addWidget(self.result_status, 1)
        self.log_toggle = QToolButton()
        self.log_toggle.setText('Show Log')
        self.log_toggle.setCheckable(True)
        self.log_toggle.toggled.connect(self._toggle_log)
        result_row.addWidget(self.log_toggle)
            ...
            try:
                ...
                if version != self._version:
                    ...
                    if topology(elements) != topology(self._elements):
                        ...
                        self.devices.clear()
                        self.device_summary.setText('Select a device to view its parameters')
