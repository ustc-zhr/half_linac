...
class lattice_parser:
    '''
    Parser for elegant lattice *.lte file
    get 'lattice' and 'trackline' in *.lte file, saved as dicts and list
    '''
    
    def __init__(self, fileName=None, lineName="ALL"):
        if fileName==None:
            pass
        else:
            ...
            self.lattice = None        # {'elem1_name':{elem1}, 'elem2_name':{elem2}, ...}
            self.trackline_names_list = None  # [elem1_name, elem2_name, ...]
           
    def get_lattice_tracklinenameslist(self):
        '''
        do not add this func in the init, as one.ele file has no lattice lines
        '''

        lines = self.get_brieflines()
        
        # get all elements in a nested dict
        ...
        for line in lines:
            ...
            if line_type == 'MATH_EXPR':
                ...
                for para in tmp[1:]:
                    ...
                    para_value = para.split('=')[1]   #variables use lower case
                    ...
                    elem[para_name] = str(para_value).replace(' ','')
    ...
    def rpn_cal(self, rpn_str:str):
        ...
        for c in rpn_str.split():
            if c in '+-*/':
                ...
                stack.append(c)
        return stack
    def randn(self,sigma,cutat):
        '''
        generate gauss random value, cutoff at cutat
        '''
