...
if __package__ in {None, ""}:
    ...
    from jitter_analysis.domain.types import MultiKnobStepRecord, RunMode, RunResult, RunStatus, SampleRecord, ScanStepRecord
    ...
    from jitter_analysis.gui.plots.theme import apply_plot_theme
    from jitter_analysis.gui.theme import apply_app_theme, available_themes, current_theme_id, theme_label
    ...
    from ..domain.types import MultiKnobStepRecord, RunMode, RunResult, RunStatus, SampleRecord, ScanStepRecord
    ...
    from .plots.theme import apply_plot_theme
    from .theme import apply_app_theme, available_themes, current_theme_id, theme_label
...
class MainWindow(_MainWindowBase):
    ...
    def _build_actions(self):
        ...
        for spec in available_themes():
            action = QtWidgets.QAction(spec.label, self)
            action.setCheckable(True)
            action.triggered.connect(
                lambda checked, theme_id=spec.theme_id: checked and self._set_gui_theme(theme_id)
            )
            self.theme_action_group.addAction(action)
            self._theme_actions[spec.theme_id] = action
    ...
    def _build_menu_bar(self) -> None:
        ...
        for spec in available_themes():
            action = self._theme_actions.get(spec.theme_id)
    ...
    def _build_run_tab(self):
        ...
        inner.addWidget(
            self._create_page_hint(
                "Start or stop the run here and watch the live trend update in real time."
            )
        )
        ...
        self.run_check_button.setMinimumHeight(34)
        self.run_start_button.setMinimumHeight(36)
        self.run_stop_button.setMinimumHeight(34)
        self._apply_action_button_role(self.run_check_button, "subtle")
        self._apply_action_button_role(self.run_start_button, "primary")
        self._apply_action_button_role(self.run_stop_button, "danger")
        controls_row.addWidget(self.run_check_button)
        controls_row.addWidget(self.run_start_button)
        controls_row.addWidget(self.run_stop_button)
        controls_row.addStretch(1)
        inner.addLayout(controls_row)
    ...
    def _build_analysis_page(self):
        ...
        box = self._create_section_group("Analysis")
        inner = QtWidgets.QVBoxLayout(box)
        header = QtWidgets.QHBoxLayout()
        header.addStretch(1)
        self.analysis_axis_label = QtWidgets.QLabel("Single Knob X Axis")
        header.addWidget(self.analysis_axis_label, 0)
        ...
        header.addWidget(self.analysis_axis_combo, 0)
        self.analysis_open_run_button = QtWidgets.QPushButton("Run Browser...")
        self._apply_action_button_role(self.analysis_open_run_button, "subtle")
        header.addWidget(self.analysis_open_run_button, 0)
    ...
    def _build_mode_status_banner(self):
        ...
        frame.setStyleSheet(
            ...
            QFrame#modeStatusBanner[tone="subtle"] {
                background: #f8fafc;
    ...
    def _build_jitter_summary(self):
        ...
        self.jitter_table.setHorizontalHeaderLabels(
            ["PV", "Count", "Mean", "Std", "RMS", "P2P", "Min", "Max", "Unit"]
        )
        ...
        self.jitter_table.horizontalHeader().setStretchLastSection(True)
    ...
    def _build_log_section(self):
        ...
        self.log_toggle_button.setToolButtonStyle(QtCore.Qt.ToolButtonTextOnly)
        self.log_toggle_button.toggled.connect(self._toggle_log_view)
    ...
    def _populate_setup_browser(self, dialog: SetupBrowserDialog) -> None:
        ...
        entries = self._list_saved_setups(root_dir)
    ...
    def _list_saved_setups(self, root_dir: str | Path) -> list[dict[str, object]]:
        root = Path(root_dir)
        if not root.exists():
            return []
        ...
        for path in root.rglob("*.json"):
            ...
            saved_at = self._parse_optional_datetime(payload.get("saved_at"))
            entries.append(
                {
                    "path": str(path.resolve()),
                    "saved_at": saved_at,
                    "saved_at_text": saved_at.strftime("%Y-%m-%d %H:%M:%S") if saved_at else path.stem,
                    "task_mode": self._mode_display_name(str(payload.get("task_mode", ""))),
    ...
    @staticmethod
    def _run_browser_scope_kind(root_dir: str | Path) -> str:
    ...
    def _populate_run_browser(self, dialog: RunBrowserDialog) -> None:
        ...
        dialog.set_root_dir(root_dir)
        scope_kind = self._run_browser_scope_kind(root_dir)
        try:
    ...
    def _load_saved_run_path(self, path: str | Path) -> bool:
        ...
        if not self._ensure_loaded_run_config(result):
            return False

        self._apply_loaded_run_result(result, Path(path))
        return True

    def _load_config_path(self, path: str | Path, quiet: bool = False) -> bool:
        try:
    ...
    def _ensure_loaded_run_config(self, result: RunResult) -> bool:
        config_path = str(result.metadata.config_path or "").strip()
        if config_path:
            current_path = str(self.loaded_config.source_path) if self.loaded_config is not None else ""
            if current_path != config_path:
                loaded = self._load_config_path(config_path, quiet=True)
                if not loaded:
                    QtWidgets.QMessageBox.critical(
                        self,
                        "Open Saved Run",
                        f"Could not load PV library recorded in the run:\n{config_path}",
                    )
                    self.append_log(f"Offline run load failed: missing PV library {config_path}")
                    return False
        ...
        ok, message = self._validate_loaded_run_config(result)
        if not ok:
    ...
    def _validate_loaded_run_config(self, result: RunResult) -> tuple[bool, str]:
        if self.loaded_config is None:
        ...
        available_object_ids = {obj.id for obj in self.loaded_config.objects}
        ...
        available_knob_ids = {knob.id for knob in self.loaded_config.knobs}
    ...
    def _apply_loaded_run_result(self, result: RunResult, path: Path) -> None:
        ...
        self.status_panel.set_sample(str(len(self.current_run_records)), tone="subtle")
        ...
        if self.current_run_records and any(sample.batch_index is None for sample in self.current_run_records):
            self.append_log(
    ...
    def _apply_loaded_run_selection(self) -> None:
        ...
        selected_object_ids = list(self.current_run_details.get("target_object_ids", []))
        if not selected_object_ids:
            known_object_ids = {obj.id for obj in self.loaded_config.objects}
            seen = []
            for sample in self.current_run_records:
                if sample.pv_id in known_object_ids and sample.pv_id not in seen:
                    seen.append(sample.pv_id)
            selected_object_ids = seen

        selected_knob_ids: list[str] = []
        active_knob_id = None
        if self.current_run_mode == RunMode.KNOB_SCAN:
            knob_id = str(self.current_run_details.get("knob_id", "")).strip()
            if knob_id:
                selected_knob_ids = [knob_id]
                active_knob_id = knob_id
        elif self.current_run_mode == RunMode.MULTI_KNOB_RANDOM:
            knob_ranges = self.current_run_details.get("knob_ranges", [])
            selected_knob_ids = [
        ...
        self.state.selected_object_ids = selected_object_ids
        self.state.selected_knob_ids = selected_knob_ids
        self.state.active_knob_id = active_knob_id
        self._refresh_selected_pvs()
    ...
    def _apply_loaded_run_parameters(self) -> None:
        details = self.current_run_details
        if self.current_run_mode == RunMode.TIMED_ACQUISITION:
            if "sample_interval_sec" in details:
                self.scan_panel.interval_spin.setValue(float(details["sample_interval_sec"]))
            if "sample_count" in details:
                self.scan_panel.count_spin.setValue(int(details["sample_count"]))
            return

        if self.current_run_mode == RunMode.KNOB_SCAN:
            if "settle_delay_sec" in details:
                self.scan_panel.settle_spin.setValue(float(details["settle_delay_sec"]))
            if "sample_interval_sec" in details:
                self.scan_panel.scan_sample_interval_spin.setValue(float(details["sample_interval_sec"]))
            if "sample_count_per_step" in details:
                self.scan_panel.step_sample_spin.setValue(int(details["sample_count_per_step"]))
            if "restore_initial_value" in details:
                self.scan_panel.restore_check.setChecked(bool(details["restore_initial_value"]))
            ...
            if isinstance(scan_values, list) and scan_values:
                ...
                self.scan_panel.manual_scan_values_edit.setText(
                    ", ".join(f"{float(value):.6g}" for value in scan_values)
                )
            return

        if "settle_delay_sec" in details:
            self.scan_panel.random_settle_spin.setValue(float(details["settle_delay_sec"]))
        if "sample_interval_sec" in details:
            self.scan_panel.random_sample_interval_spin.setValue(float(details["sample_interval_sec"]))
        if "sample_count_per_point" in details:
            self.scan_panel.random_samples_per_point_spin.setValue(int(details["sample_count_per_point"]))
        if "num_points" in details:
            self.scan_panel.random_point_count_spin.setValue(int(details["num_points"]))
        if "seed" in details:
            self.scan_panel.random_seed_edit.setText(str(details["seed"]))
        if "restore_initial_values" in details:
            self.scan_panel.random_restore_check.setChecked(bool(details["restore_initial_values"]))
        distribution = str(details.get("distribution", "")).strip()
        if distribution:
            index = self.scan_panel.random_distribution_combo.findData(distribution)
            if index >= 0:
        ...
        if isinstance(knob_ranges, list) and knob_ranges:
            ...
            self.scan_panel.set_random_knob_state(state)
    ...
    def _apply_loaded_config(self) -> None:
        ...
        knob_ids = {knob.id for knob in self.loaded_config.knobs}
        object_ids = {obj.id for obj in self.loaded_config.objects}
        self.state.selected_knob_ids = [
            knob_id for knob_id in self.state.selected_knob_ids if knob_id in knob_ids
        ]
        self.state.active_knob_id = (
            self.state.active_knob_id if self.state.active_knob_id in knob_ids else None
        )
        ...
        self.state.selected_object_ids = [
            object_id for object_id in self.state.selected_object_ids if object_id in object_ids
        ]
        ...
        self.scan_panel.interval_spin.setValue(self.loaded_config.defaults.acquisition.sample_interval_sec)
        ...
        self.scan_panel.scan_sample_interval_spin.setValue(self.loaded_config.defaults.acquisition.sample_interval_sec)
        ...
        self.scan_panel.random_sample_interval_spin.setValue(
            self.loaded_config.defaults.acquisition.sample_interval_sec
        )
    ...
    def refresh_random_preview(self) -> None:
        ...
        preview_limit = min(len(target_steps), 20)
        lines = []
        ...
        if len(target_steps) > preview_limit:
            lines.append(f"... {len(target_steps) - preview_limit} more point(s)")

        summary = (
            f"{len(target_steps)} random point(s) across {len(knob_ranges)} knob(s)"
            f"  |  distribution={config['distribution']}  |  seed={seed}"
        )
        detail = ", ".join(
        ...
        self.scan_panel.set_random_preview(lines, summary=summary, detail=detail)
    ...
    def refresh_scan_preview(self, *args, force_live_center: bool = False) -> None:
        ...
        summary = (
            f"{len(values)} point(s) for {active_knob.name}: "
            f"{min(values):.6g} to {max(values):.6g} {active_knob.unit}"
        )
        if len(values) >= 2:
        ...
        if mode == "symmetric_points":
            try:
                ...
                detail = f"Preview center from {active_knob.name}: {center:.6g} {active_knob.unit}"
            except ValueError:
                detail = ""

        self.scan_panel.set_preview_values(values, summary=summary, detail=detail)

    def _mode_display_name(self, mode: str | None = None) -> str:
        value = mode or self.scan_panel.task_mode()
        labels = {
            "timed_acquisition": "Monitor",
            "single_knob_scan": "Single Knob",
            "multi_knob_random": "Random Multi-Knob",
        }
        return labels.get(value, value.replace("_", " ").title() if value else "Idle")
    ...
    def _refresh_ui_affordances(self) -> None:
        ...
        self.run_stop_button.setText(self.action_stop.text())
    ...
    def _update_mode_status_banner(self, mode: str, ready: bool, next_step: str) -> None:
        if self.state.run_status == RunStatus.RUNNING:
            ...
            tone = "warning"
            title = f"{self._mode_display_name(mode)} needs one more step"
            message = next_step
    ...
    def _analysis_mode_key(self, fallback_mode: str) -> str:
        if self._has_analysis_data and self.current_run_mode is not None:
            mapping = {
                RunMode.TIMED_ACQUISITION: "timed_acquisition",
                RunMode.KNOB_SCAN: "single_knob_scan",
                RunMode.MULTI_KNOB_RANDOM: "multi_knob_random",
            }
            return mapping.get(self.current_run_mode, fallback_mode)
        return fallback_mode

    def _single_knob_axis_name(self, knob_name: str = "") -> str:
        base_name = knob_name.strip() or "Knob Value"
        if self._single_knob_axis_source == "target":
    ...
    def _prepare_analysis_views_for_current_data(self) -> None:
        ...
        self._set_analysis_available(bool(self.current_run_records))
    ...
    def _ensure_random_seed(self, seed_text: str) -> int:
        ...
        if not text:
            seed = int(QtCore.QDateTime.currentMSecsSinceEpoch())
            self.scan_panel.set_random_seed(seed)
            return seed
        try:
            return int(text)
    ...
    def _collect_random_knob_ranges(self):
        selected_knobs = {knob.id: knob for knob in self._selected_knobs()}
        rows = self.scan_panel.random_knob_state()
        knob_ranges = []
        for knob_id, row in rows.items():
            if not bool(row.get("enabled", True)):
                continue
            knob = selected_knobs.get(knob_id)
            if knob is None:
                continue
    ...
    def start_timed_acquisition(self) -> None:
        ...
        interval_sec = float(self.scan_panel.interval_spin.value())
        sample_count = int(self.scan_panel.count_spin.value())
        if interval_sec <= 0 or sample_count <= 0:
    ...
    def start_knob_scan(self) -> None:
        ...
        sample_interval_sec = float(self.scan_panel.scan_sample_interval_spin.value())
    ...
    def start_multi_knob_random(self) -> None:
        ...
        sample_interval_sec = float(config["sample_interval_sec"])
    ...
    def _on_acquisition_batch(self, sample_index: int, batch) -> None:
        ...
        self.trend_plot.append_batch(sample_index if not batch else recorded_sample_index, batch)

        last_sample = batch[-1] if batch else None
        if last_sample is not None:
            current_value = "nan" if math.isnan(last_sample.value) else f"{last_sample.value:.6g}"
            self.status_panel.set_current(f"{last_sample.pv_id}: {current_value}", tone="subtle")
    ...
    def _rebuild_loaded_run_views(self) -> None:
        ...
        trend_x_values = {obj.id: [] for obj in selected_objects}
    ...
    def _loaded_run_object_count_hint(self) -> int:
        target_object_ids = [
            str(item).strip()
            for item in self.current_run_details.get("target_object_ids", [])
            if str(item).strip()
        ]
        ...
        seen_ids = []
        for sample in self.current_run_records:
            if sample.pv_id not in seen_ids:
                seen_ids.append(sample.pv_id)
        return len(seen_ids)
    ...
    def _populate_jitter_table(self) -> None:
        ...
        for pv_id, values in self.current_series_values.items():
            ...
            if not clean_values:
                continue
            stats = compute_jitter_stats(clean_values)
            obj = objects_by_id.get(pv_id)
            name = obj.name if obj else pv_id
            unit = obj.unit if obj else ""
            sample_indices = [
                int(item["sample_index"])
                for item in self.current_series_metadata.get(pv_id, [])
                if not math.isnan(float(item.get("value", float("nan"))))
            ]
            rows.append((pv_id, name, stats, unit))
            plot_rows.append(
                {
                    "pv_id": pv_id,
                    "label": name,
        ...
        for row_index, (pv_id, name, stats, unit) in enumerate(rows):
            values = [
                name,
                str(stats.count),
                f"{stats.mean:.6g}",
        ...
        self.jitter_table.resizeColumnsToContents()
        ...
        if plot_rows:
            ...
            try:
                ...
                del blockers
        self._set_analysis_available(bool(rows))
    ...
    def _populate_correlation_view(self) -> None:
        ...
        for pv_id, values in self.current_series_values.items():
            if pv_id not in objects_by_id:
                continue
            if sum(1 for value in values if not math.isnan(value)) < 2:
                continue
            base_name = objects_by_id[pv_id].name
            display_name = base_name
            if display_name in series_by_name:
                display_name = f"{base_name} ({pv_id})"
            series_by_name[display_name] = list(values)
            series_metadata_by_name[display_name] = {
                "values": list(values),
                "sample_indices": [
                    int(item["sample_index"]) for item in self.current_series_metadata.get(pv_id, [])
                ],
                "step_indices": [
                    item.get("step_index") for item in self.current_series_metadata.get(pv_id, [])
                ],
                "timestamps": [
                    item.get("timestamp") for item in self.current_series_metadata.get(pv_id, [])
                ],
                "pv_id": pv_id,
            }
        ...
        if len(series_by_name) < 2:
            self.correlation_plot.clear_data(
                "Need at least two read PV series with valid samples to compute correlation."
            )
            return
    ...
    def _populate_spectrum_view(self) -> None:
        ...
        samples_by_pv = {}
        for sample in self.current_run_records:
            if sample.pv_id not in objects_by_id:
                continue
            if math.isnan(sample.value):
                continue
            samples_by_pv.setdefault(sample.pv_id, []).append(sample)

        series_sources = []
        seen_names: set[str] = set()
        for pv_id, samples in samples_by_pv.items():
            if len(samples) < 2:
                continue
    ...
    def _highlight_correlation_point(self, payload) -> None:
        ...
        if step_index is not None:
            indices = []
            for entries in self.current_series_metadata.values():
                for item in entries:
