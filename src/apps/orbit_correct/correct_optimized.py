...
class OrbitCorrector:
    ...
    def _get_avg_readings(self, pv_list: List[PV]) -> List[float]:
        ...
        return [r / samples_perstep for r in results]
...
if __name__ == '__main__':
    try:
        ...
        if sys.argv[1] == "start_cor":
            ...
            target_BPMx_values = [float(x)*1e-3 for x in sys.argv[7].split(',')]
            target_BPMy_values = [float(y)*1e-3 for y in sys.argv[8].split(',')]
