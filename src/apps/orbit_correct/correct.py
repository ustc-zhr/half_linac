...
#响应矩阵方法进行轨道校正
    ...
    def correct_one_to_one(self):
        ...
        for j in range(len(self.bpm_list_target)):
            ...
            #根据响应函数以及目标轨道调整上游校正子
            hcorrVal = hcorrVal + (self.target_BPMx_values[j] - xbpmVal) / Rx
            vcorrVal = vcorrVal + (self.target_BPMy_values[j] - ybpmVal) / Ry
            ...
            ybpmVald = self.pvBPMy[j].get()
            loop = 0
            while abs(self.target_BPMx_values[j] - xbpmVald)>self.cor_accuracy or abs(self.target_BPMy_values[j] - ybpmVald)>self.cor_accuracy:    
                loop += 1
                print('cor loop ',loop,'for',self.bpm_list_target[j],'\n')
                # print(hcorrVal, self.target_BPMx_values[j], xbpmVal, Rx)
                hcorrVal = hcorrVal + (self.target_BPMx_values[j] - xbpmVald) / Rx
                vcorrVal = vcorrVal + (self.target_BPMy_values[j] - ybpmVald) / Ry
                self.pvCORx[j].put(hcorrVal)
                self.pvCORy[j].put(vcorrVal)
                time.sleep(self.timer_interval)
                xbpmVald = self.pvBPMx[j].get()
                ybpmVald = self.pvBPMy[j].get()
                print("postion:", xbpmVald, ybpmVald)
