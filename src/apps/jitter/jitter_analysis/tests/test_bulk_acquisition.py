...
def test_sample_objects_can_reduce_waveform_pv_to_mean():
    class _StubClient:
        def read_many(self, pv_names: list[str]) -> list[ReadResult]:
            assert pv_names == ["PV:BPM:WAVEFORM"]
            return [ReadResult(value=[1.0, 2.0, 3.0, 4.0], connected=True)]

    client = _StubClient()
    sampler = AcquisitionSampler(client)
    obj = _object_spec("bpm_waveform_mean", "PV:BPM:WAVEFORM")
    obj.value_reducer = "mean"

    sample = sampler.sample_object(obj)

    assert sample.pv_id == "bpm_waveform_mean"
    assert sample.value == 2.5
    assert sample.connected is True


def test_sample_batch_splits_scalar_and_waveform_objects():
    class _StubClient:
        def read_many(self, pv_names: list[str]) -> list[ReadResult]:
            assert pv_names == ["PV:BPM:X", "PV:SCOPE:WAVE"]
            return [
                ReadResult(value=1.25, connected=True),
                ReadResult(value=[0.0, 1.0, 0.5, -0.25], connected=True),
            ]

    client = _StubClient()
    sampler = AcquisitionSampler(client)
    scalar = _object_spec("bpm_x", "PV:BPM:X")
    waveform = _object_spec("scope_wave", "PV:SCOPE:WAVE")
    waveform.capture_mode = "waveform"
    waveform.waveform_sample_interval_sec = 1.0e-9

    batch = sampler.sample_batch([scalar, waveform], batch_index=7)

    assert len(batch.scalar_samples) == 1
    assert len(batch.waveform_samples) == 1
    assert batch.scalar_samples[0].pv_id == "bpm_x"
    assert batch.scalar_samples[0].value == 1.25
    assert batch.waveform_samples[0].pv_id == "scope_wave"
    assert batch.waveform_samples[0].values == [0.0, 1.0, 0.5, -0.25]
    assert batch.waveform_samples[0].waveform_sample_interval_sec == 1.0e-9
    assert batch.waveform_samples[0].batch_index == 7


def test_sample_objects_rejects_waveform_capture_objects():
    class _StubClient:
        def read_many(self, pv_names: list[str]) -> list[ReadResult]:
            raise AssertionError("sample_objects should reject waveform capture before reading")
    ...
    try:
        sampler.sample_objects([waveform])
        raised = False
    except ValueError as exc:
        raised = True
        assert "sample_batch" in str(exc)
