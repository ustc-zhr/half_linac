...
class BOOptimizer:
    ...
    def __init__(self, func, bounds, acq: str = "UCB", acq_para: float = 2.576, random_state: int = None):
        """
        Parameters:
        ...
        if acq == "UCB": #kappa
            self.acquisition_function = acquisition.UpperConfidenceBound(acq_para)
        
        if acq == "EI":  #xi
            self.acquisition_function = acquisition.ExpectedImprovement(acq_para)
        
        if acq == "PI":  #xi
            self.acquisition_function = acquisition.ProbabilityOfImprovement(acq_para)
...
class TargetIoc:
    def __init__(self, knobs_list=None, obj_pvnames=None, obj_weights=None, obj_samples=None, obj_math=None, interval=None):
        # EPICS相关
...
if __name__ == "__main__":
    ...
    try:
        ...
        if sys.argv[1] == "start_opt":
            ...
            # BO setup
            init_points = int(sys.argv[5])
            maxIt = int(sys.argv[6])
            acq = sys.argv[7]
            acc_para = float(sys.argv[8])
            # obj
            interval = float(sys.argv[9])
            obj_pvnames = sys.argv[10].split(',')
            obj_weights = [float(x) for x in sys.argv[11].split(',')]
            obj_samples = [int(x) for x in sys.argv[12].split(',')]
            obj_samples = max(obj_samples)
            obj_math = sys.argv[13].split(',')

            # 建立与目标函数的通道
            objhub = TargetIoc(knobs_list=knobs_list, obj_pvnames=obj_pvnames, obj_weights=obj_weights, obj_samples=obj_samples, obj_math=obj_math, interval=interval)
            knobs_init = objhub.init_knob_value()
            vrange_b = [x + y for x, y in zip(knobs_minus, knobs_init)]
            vrange_u = [x + y for x, y in zip(knobs_plus, knobs_init)]
            vrange = np.array([vrange_b, vrange_u]).T
            ...
            best = optimizer.maximize(n_iter=maxIt, init_points=5, save_path="bo_record.json")
            print("Optimal:", best)
