...
class RSimplex:
    ...
    def __init__(self, func = None, start_point=None, vrange=None, knobs_list=None,
        ...
        obj_pvnames: 目标函数值PV名
        obj_weights: 目标函数值权重
        obj_samples: 目标函数值采样次数
        obj_math: 多次采样下目标函数值统计方式
        """
        self.func = func
...
if __name__ == "__main__":
    ...
    try:
        ...
        if sys.argv[1] == "start_opt":
            ...
            # 创建优化器实例
            start_point = 0.5*np.ones(len(knobs_list)) #归一化的初始点
            vrange = np.array([knobs_minus, knobs_plus]).T #基于初始点的探索空间加减范围(未归一化的)
  
            # 创建优化器
            optimizer = RSimplex(
                start_point = start_point, vrange = vrange, knobs_list = knobs_list,
                step_size = step, noise = noise, interval = interval, 
                tol=1e-6, max_iters=maxIt, max_evaluations=1000,
                obj_pvnames=obj_pvnames, obj_weights=obj_weights, obj_samples=obj_samples, obj_math=obj_math 
            )
            
            # 执行优化
            x_norm_opt, history = optimizer.simplexmain()
