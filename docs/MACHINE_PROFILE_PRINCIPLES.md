...
- When `kind` alone is still too broad, prefer filtering by required logical channel before introducing app-specific metadata.
...
- Keep app workflow JSON for small non-derivable facts such as image geometry, scan defaults, or one recommended default element.
...
2. If needed, filter by required logical channel.
3. If needed, add `plane`.
4. If still needed, add a small number of simple tags.
5. Only introduce finer app-specific roles when the simpler choices are proven insufficient.
