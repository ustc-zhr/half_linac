...
class RSimplex:
    ...
    def __init__(self, func, noise_level=0.01, M1=1.4, M2=2.0, 
                 max_iters=1000, max_evaluations=1000, max_samples=3, rebuild_fraction=0.6,
                 obj_pvs=None, param_pvs=None):
        """
        初始化鲁棒单纯形优化器
        ...
        # EPICS相关
        self.obj_pvs = obj_pvs if obj_pvs is not None else []
        self.param_pvs = param_pvs if param_pvs is not None else []
        self.use_epics = len(self.obj_pvs) > 0 and len(self.param_pvs) > 0
    ...
    def __evaluate_epics(self, x):
        ...
        pv = epics.PV(self.obj_pvs)
        total = -1*pv.get(self.obj_pvs)
        time.sleep(0.5) 
    ...
    def _evaluate(self, x):
        ...
        evaluate_value = self.__evaluate_epics if self.use_epics else self.func(x)
        self.history['eval_values'].append(evaluate_value)
