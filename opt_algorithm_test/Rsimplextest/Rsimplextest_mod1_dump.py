...
class RSimplex:
    """
    鲁棒单纯形算法(Robust Simplex Algorithm)实现
    基于PHYSICAL REVIEW ACCELERATORS AND BEAMS 21, 104601 (2018)
    """
    
    def __init__(self, func, noise_level=0.01, M1=1.4, M2=2.0, 
                 max_evaluations=1000, max_samples=4, rebuild_fraction=0.5):
        """
        初始化鲁棒单纯形优化器
    ...
    def _evaluate(self, x):
        ...
        self.history['eval_values'].append(self.func(x))
        return self.func(x)
    
    def _sample_point(self, x, min_samples=1):
        """
        对点进行多次采样取平均以减少噪声影响
        ...
        for _ in range(min_samples):
            samples.append(self._evaluate(x))
    ...
    def _compare(self, f1, std1, f2, std2, x1=None, x2=None, current_samples=1):
        """
        统计比较两个函数值
        返回:
        -1: f1 < f2
        ...
        std_diff = np.sqrt(2/current_samples)*self.sigma
        ...
        # 如果无法确定且采样次数不足，增加采样
        if current_samples < self.max_samples and x1 is not None and x2 is not None:
            # 对两个点都增加采样
            print('增加采样')
            f1_new, std1_new, count1 = self._sample_point(x1, min_samples=current_samples+1)
            f2_new, std2_new, count2 = self._sample_point(x2, min_samples=current_samples+1)
            print('增加采用后:', f1_new, std1_new, f2_new, std2_new)
            return self._compare(f1_new, std1_new, f2_new, std2_new, x1, x2, current_samples+1)
    ...
    def optimize(self, start, step_size=0.1, tol=1e-6):
        """
        执行优化过程
        ...
        """
        n = len(start)
        simplex = self._initialize_simplex(start, step_size)
        f_values = []
        f_stds = []
        sample_counts = []
        ...
        for point in simplex:
            f, std, count = self._sample_point(point) # sample 默认1次
            f_values.append(f)
            f_stds.append(std)
        ...
        while self.history['evaluations'] < self.max_evals:
            ...
            for i in range(len(simplex)-2, 0, -1):
                # 检查是否与最大值足够接近
                print("顶点组G_max: ", i)
                print('input:', f_values[i], f_stds[i], f_values[-1], f_stds[-1])
                print('input:', simplex[i], simplex[-1])
                comp = self._compare(f_values[i], f_stds[i],
                                    f_values[-1], f_stds[-1],
                                    simplex[i], simplex[-1],
                                    min(sample_counts[i], sample_counts[-1]))
                if comp == 0:  # 无法区分
                    G_max.append(i)
                else:
                    break
                ...
                if comp_best == -1:  # fr < f1
                    ...
                    if comp_fn == -1:  # fr < fn
                        ...
                        if comp_fn == 1:  # fr > fn
                            ...
                            values = [
                                f_values[vertex_idx],
                                f_values[vertex_idx] if len(points) < 2 else self._evaluate(points[1]),
                                self._evaluate(Xc),
                                foc,
                                fr
                            ]
            ...
            # 检查评估次数限制
            if self.history['evaluations'] >= self.max_evals:
                break
...
if __name__ == "__main__":
    ...
    start_point = np.zeros(dim)  # 初始点(0,0,...,0)
    step_size = 0.5  # 初始步长
    ...
    # 执行优化
    best_point, history = optimizer.optimize(start_point, step_size)
    ...
    print(f"历史最佳值: {min(history['best_values']):.6f}")
