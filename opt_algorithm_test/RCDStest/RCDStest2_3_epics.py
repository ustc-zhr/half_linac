...
class PowellOptimizer:
    def __init__(self, func=None, x0=None, vrange=None, step=0.1, Dmat0=None,
                 ...
                 obj_pvs=None, param_pvs=None, timeout=5.0):
        """
        参数:
    ...
    def _epics_func(self, x):
        ...
        # 获取目标函数值
        total = 0.0
        for pvname in self.obj_pvs:
            pv = epics.PV(pvname)
            if not pv.wait_for_connection(timeout=self.timeout):
                raise RuntimeError(f"无法连接目标PV: {pvname}")
...
if __name__ == "__main__":
    ...
    # x0 = np.random.rand(vrange.shape[0])# 生成随机初始点(归一化到[0,1]范围)
    x0 = [0.5, 0.7]
